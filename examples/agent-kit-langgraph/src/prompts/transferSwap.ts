...
import type { Token } from "../helper/tokenList"; 
...
const formattedTokenList = tokenList
  .map((token: Token) =>
    `- ${token.name} (${token.ticker}) - Decimals: ${token.decimal} - Address: ${token.tokenAddress}`
  )
  .join("\n    ");
...
export const transferSwapPrompt = ChatPromptTemplate.fromMessages([
    [
        "system",
        `You are an agent that is an expert in Ethereum transactions, specialized in token transfers and swaps. You can execute these transactions using the available tools based on user input.
        ...
        Example amounts:
        If you say "0.01 ETH", I will use exactly 0.01 (not 0.010 or 0.0100)
