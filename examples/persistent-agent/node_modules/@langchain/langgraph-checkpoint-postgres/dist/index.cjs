    ...
    async setup() {
        ...
        try {
            ...
            try {
                ...
                if (error?.message.includes(`relation "${SCHEMA_TABLES.checkpoint_migrations}" does not exist`)) {
                    version = -1;
                }
                else {
                    throw error;
    ...
    _dumpWrites(threadId, checkpointNs, checkpointId, taskId, writes) {
        return writes.map(([channel, value], idx) => {
            ...
            return [
                ...
                checkpointId,
                taskId,
                langgraph_checkpoint_1.WRITES_IDX_MAP[channel] !== undefined ? langgraph_checkpoint_1.WRITES_IDX_MAP[channel] : idx,
                channel,
                type,
